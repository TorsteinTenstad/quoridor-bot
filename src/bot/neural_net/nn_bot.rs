...
use crate::data_model::PIECE_GRID_HEIGHT;
use crate::data_model::PIECE_GRID_WIDTH;
use crate::data_model::Player;
use crate::data_model::PlayerMove;
use crate::data_model::WALL_GRID_HEIGHT;
use crate::data_model::WALL_GRID_WIDTH;
use crate::data_model::WallOrientation;
...
pub const ACTIONS: usize = ALL_MOVES.len(); // Total moves: 16 pawn moves + 162 wall placements (8x8 grid x 2 orientations + some boundary)
...
pub fn get_move(game: &Game, network: &QuoridorNet, temperature: f32) -> PlayerMove
{
    ...
    if legal_moves.is_empty() {
        panic!("No legal moves available for player {:?}", game.player);
    }
